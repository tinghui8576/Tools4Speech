...
if df is not None and st.session_state.selected_idx is not None:
    ...
    audio_bytes = slice_wav_bytes(str(row_data["origin_filename"]), selected_start, selected_end )
    if audio_bytes:
    ...
    with st.container(border=True):
        ...
        if any(x in md_cols for x in ["arousal", "valence", "dominance"]) and "arousal__valence__dominance" not in st.session_state.hidden_fields:
            # current_arousal = _resolve_value("arousal", row_data, 0.5)
            # current_valence = _resolve_value("valence", row_data, 0.5)
            # current_dominance =_resolve_value("dominance", row_data, 0.5)
            with c_emo[1]:
    ...
    if submit_save:
        ...
        # df.at[idx, "age"] = final_age if "age" not in st.session_state.hidden_fields else np.nan
